<div class="group/game-card transition-all duration-300 overflow-hidden rounded-xl sm:rounded-2xl md:rounded-3xl bg-vulcan-800 bg-opacity-50 shadow-lg border border-vulcan-700 hover:shadow-2xl">
    <div class="flex items-center relative aspect-square">
        ...
        <?php if (auth()->isLoggedIn()) : ?>
            <?php if (($game->creator == auth()->user()->id) || ($game->is_verified && auth()->isAdmin())) : ?>
            ...
            <?php endif ?>
            <?php if (($game->creator == auth()->user()->id)) : ?>
        ...
        <?php if (!$game->is_verified && auth()->isUser()) : ?>
            <span class="absolute <?= $game->creator == auth()->user()->id ? "top-10" : "top-0" ?> left-0 z-20 text-sm text-center py-1.5 px-3 font-medium rounded-br-xl transition focus:outline-none bg-orange-600 bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-50 group-hover/game-card:bg-opacity-90 text-slate-200 hover:bg-orange-500 hover:border-orange-500 hover:text-white shadow-lg">
    ...
    <div class="flex flex-col w-full bg-vulcan-800 px-2 md:px-3 py-2 md:py-3 border-t h-full border-vulcan-600">
        ...
        <div class="flex flex-col w-full mt-2">
            ...
            <div class="flex flex-row items-center w-full mt-2">
                <a href="<?= base_url("/user/$game->creator_username") ?>" class="flex flex-row items-center gap-2 border-2 border-transparent text-sm transition outline-none">
                    <span class="h-7 w-7 rounded-full">
                        <img class="h-7 w-7 rounded-full object-cover" src="<?= $game->creator_photo ?>" alt="<?= $game->creator_name ?>">
                    </span>
                    <span class="flex flex-col text-start">
                        ...
                        <span class="text-xs text-slate-300 -mt-1">@<?= $game->creator_username ?></span>
                    </span>
                </a>
            </div>
        </div>
